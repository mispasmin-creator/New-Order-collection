...
export default function ArrangeLogistics({ user }) {
  ...
  const firmOptions = useMemo(() => {
    const firms = [
      ...new Set(orders.map((order) => order["Firm Name"]).filter(Boolean)),
    ];
    return ["all", ...firms];
  }, [orders]);
  ...
  const partyOptions = useMemo(() => {
    const parties = [
      ...new Set(orders.map((order) => order["Party Names"]).filter(Boolean)),
    ];
    return ["all", ...parties];
  }, [orders]);
  ...
  const displayOrders = useMemo(() => {
    ...
    if (filterParty !== "all") {
      filtered = filtered.filter(
        (order) => order["Party Names"] === filterParty,
      );
    }
  ...
  const handleExport = () => {
    const dataToExport = displayOrders.map((order) => ({
      ...order,
      "Type of transporting": order["Type Of Transporting"] || "",
    }));

    exportToExcel(dataToExport, "ArrangeLogistics");
  };
  ...
  const addOption = () => {
    if (transporterOptions.length >= 10) {
      toast({
        description: "Max 10 transporter options allowed",
        variant: "destructive",
      });
      return;
    }
  ...
  const removeOption = (optionIndex) => {
    if (transporterOptions.length <= 1) {
      toast({
        description: "At least one transporter option required",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleSubmit = async () => {
    const validOptions = transporterOptions.filter((o) =>
      o.transporter_name.trim(),
    );
    ...
    if (missingRate) {
      toast({
        title: "Validation Error",
        description:
          "Entered Rate is required for Fixed and Per MT transporter options.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      selectedRows.forEach((order, pIdx) => {
        const remainingToApprove =
          ...
          (parseFloat(order.logistics_approved_qty) || 0);
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600">
            Assign a transporter for each product in a PO
          </p>
        </div>
                  ...
                  return (
                    <Fragment key={group.key}>
                      <TableRow
                        ...
                        onClick={() =>
                          setExpandedPO(isExpanded ? null : group.key)
                        }
                      >
                                    ...
                                    <div>
                                      ...
                                      <p className="font-medium text-green-700">
                                        {order["Total PO Basic Value"]
                                          ? `₹${Number(order["Total PO Basic Value"]).toLocaleString()}`
                                          : "—"}
                                      </p>
                                    </div>
                                    <div>
          ...
          <DialogHeader className="p-6 pb-4 border-b">
            ...
            <div className="flex flex-col gap-2 mt-4 text-sm">
              <div className="flex items-center justify-between mb-1">
                ...
                >
                  {selectedProductIds.size === selectedGroup?.rows.length
                    ? "Unselect All"
                    : "Select All"}
                </button>
              </div>
                      ...
                      <span className="text-[10px] font-bold text-gray-500">
                        {Math.max(
                          ...
                          (parseFloat(row.Quantity) || 0) -
                            (parseFloat(row.logistics_approved_qty) || 0),
                        )}{" "}
                        {row["Type Of Measurement"] || "Tons"}
                      </span>
                    </div>
                  </div>
                ))}
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs text-gray-500">
                  Provide options for the entire PO (max 10).
                </p>
              </div>
            ...
            <div className="grid gap-4 lg:grid-cols-3">
              {transporterOptions.map((opt, oIdx) => {
                const totalRemainingQty =
                  selectedGroup?.rows.reduce((sum, r) => {
                    const rem =
                      (parseFloat(r.Quantity) || 0) -
                      (parseFloat(r.logistics_approved_qty) || 0);
                    return sum + Math.max(0, rem);
                  }, 0) || 1;
                const entered = parseFloat(opt.cost) || 0;
                const isFixed = opt.rateType === "Fixed";
                const estTotalCost = isFixed
                  ? entered
                  : entered * totalRemainingQty;
                const estRatePerMt = isFixed
                  ? entered / totalRemainingQty
                  : entered;
                const isExFactory =
                  opt.transporter_name === "Ex Factory Transporter";
                  ...
                  >
                    ...
                    <div className="space-y-1">
                      ...
                      <Select
                        ...
                        onValueChange={(v) =>
                          handleOptionChange(oIdx, "transporter_name", v)
                        }
                      >
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <Select
                          ...
                          onValueChange={(value) =>
                            handleOptionChange(oIdx, "rateType", value)
                          }
                      ...
                      <div>
                        <Label className="block mb-1 text-[10px] font-medium text-gray-400 uppercase tracking-wider">
                          ...
                          {isExFactory && (
                            <span className="text-amber-500 ml-1 normal-case text-[9px]">
                              (Optional)
                            </span>
                          )}
                        </Label>
                        <Input
                          value={opt.cost}
                          onChange={(e) =>
                            handleOptionChange(oIdx, "cost", e.target.value)
                          }
                          className={`text-sm border-gray-200 h-9 bg-white ${
                            !isExFactory && !opt.cost
                              ? "border-red-200 focus-visible:ring-red-400"
                              : ""
                          }`}
                    ...
                    {opt.cost && opt.rateType && (
                      <div className="pt-3 border-t border-gray-100 space-y-2">
                        <div className="flex justify-between items-center p-2 rounded-lg bg-blue-50/50 border border-blue-100/50">
                          ...
                          {(() => {
                            const totalSelectedRemainingQty =
                              selectedGroup?.rows
                            ...
                            const ratePerMt = isFixedVal
                              ? enteredVal / totalSelectedRemainingQty
                              : enteredVal;
                            return (
                              <span className="text-sm font-bold text-blue-700">
                                ₹
                                {ratePerMt.toLocaleString("en-IN", {
          ...
          <div className="sticky bottom-0 bg-white border-t p-4 px-6 flex justify-end gap-3 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
            ...
            <Button
              ...
              disabled={
                ...
                transporterOptions.every((o) => !o.transporter_name.trim())
              }
