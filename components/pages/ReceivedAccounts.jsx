...
export default function ReceivedInAccountsPage({ user, onNavigate }) {
  ...
  // Reset party filter when tab changes
  useEffect(() => {
    setFilterParty("all")
  }, [activeTab])
  ...
  // Get unique party names for filter based on active tab
  const partyOptions = useMemo(() => {
    const currentList = activeTab === "pending" ? pendingOrders : historyOrders
    const parties = [...new Set(currentList.map(order => order.partyName).filter(Boolean))]
    return ["all", ...parties]
  ...
  // Apply party filter
  const applyPartyFilter = (ordersList) => {
    if (filterParty === "all") return ordersList
    return ordersList.filter(order => order.partyName === filterParty)
  }

  const displayOrders = activeTab === "pending"
    ? applyPartyFilter(applyStatusFilter(searchFilteredOrders(pendingOrders)))
    : applyPartyFilter(applyStatusFilter(searchFilteredOrders(historyOrders)))
  ...
  const handleExport = () => {
    exportToExcel(displayOrders, `ReceivedAccounts_${activeTab}`)
  }
