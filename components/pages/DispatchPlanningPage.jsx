...
export default function DispatchPlanningPage({ user }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // Ensure all POs with pending quantity and eligibility appear in pending
      const checkedSplitPoIds = new Set(mergedSplits.filter(s => s.splitStatus === STATUS_CHECKED && !s.dispatchRecordId).map(s => s.poId));
      ...
      (orderData || []).forEach(order => {
        ...
        if (pendingQty > 0 && isEligible && !checkedSplitPoIds.has(order.id)) {
          mergedSplits.push({
            id: `synthetic_${order.id}`,
            poId: order.id,
            planId: order.approved_logistics_plan_id || null,
            splitStatus: STATUS_CHECKED,
            dispatchRecordId: null,
            allocatedQty: pendingQty,
            transporterName: order["Type Of Transporting"] || "—",
            transporterRate: "",
            rate: order["Rate Of Material"] || "",
            totalPOValue: order["Total PO Basic Value"] || "",
            freight: order["Freight"] || "",
      ...
      setDispatchHistory([
        ...
        ...(orderData || []).filter(o => o.logistics_status === "Order Cancelled").map(order => ({
          ...
          dSrNumber: "—",
          deliveryOrderNo: order["DO-Delivery Order No."] || "—",
          partyPONumber: order["PARTY PO NO (As Per Po Exact)"] || "",
          partyName: order["Party Names"] || "",
          productName: order["Product Name"] || "",
      ...
      for (const group of poGroupMap.values()) {
        const isCancelled = group.orders.some(o => o.logistics_status === "Order Cancelled")
  ...
  const displayRows = useMemo(() => {
    let source = activeTab === "pending" ? pendingRows : dispatchHistory
  ...
  const filteredDashboard = useMemo(() => {
    ...
    if (filterFirm !== "all") {
      result = result.filter(g => g.firmName === filterFirm)
    }
    if (filterParty !== "all") {
  ...
  const handleExport = () => {
    const dataToExport = activeTab === "overview" ? dashboardGroups : displayRows
    exportToExcel(dataToExport, `DispatchPlanning_${activeTab}`)
  }
  ...
  const handleCancelOrder = async (group) => {
    ...
    try {
      ...
      if (poIds.length === 0) {
        toast({ title: "Cancel failed", description: "No valid order IDs found.", variant: "destructive" })
        return
      }
      ...
      // Mark any pending plans/splits as rejected
      await supabase.from("po_logistics_plans").update({ status: "Rejected" }).in("po_id", poIds)
      await supabase.from("po_logistics_splits").update({ status: "Rejected" }).in("po_id", poIds)
    ...
    try {
      ...
      await Promise.all(
        activeLines.map(async (line) => {
          ...
          if (String(finalSplitId).startsWith("synthetic_")) {
            if (!finalPlanId) {
              const { data: plan, error: planErr } = await supabase.from("po_logistics_plans").insert([{ po_id: line.poId, mode: "single", status: "Approved", created_by: "Auto Generated" }]).select("id").single();
              if (planErr) throw planErr;
              finalPlanId = plan.id;
              await supabase.from("ORDER RECEIPT").update({ logistics_status: "Approved", approved_logistics_plan_id: finalPlanId }).eq("id", line.poId);
            }
            const { data: newSplit, error: nsErr } = await supabase.from("po_logistics_splits").insert([{
              plan_id: finalPlanId,
              po_id: line.poId,
      ...
      <div className="bg-white border rounded-md shadow-sm p-4 space-y-4">
        <div className="flex flex-col md:flex-row gap-4">
          <div className="flex-1 relative">
            ...
            <Input 
              placeholder={activeTab === "overview" ? "Search PO, party or firm..." : "Search rows..."} 
              value={searchTerm} 
                        ...
                        <TableRow className="hover:bg-gray-50 cursor-pointer" onClick={() => toggleExpand(group.poNumber)}>
                          ...
                          <TableCell onClick={(e) => e.stopPropagation()}>
                            {(() => {
                              ...
                              return poPendingSplits.length > 0 && (
                                <Button size="sm" onClick={() => handleOpenGroup(pendingGroup)} className="bg-blue-600 hover:bg-blue-700 h-8 px-3 text-xs shrink-0 shadow-sm" disabled={submitting}>
                          ...
                          <TableCell className="text-right">
                            <div className="flex justify-end gap-2" onClick={(e) => e.stopPropagation()}>
                              <Button size="sm" onClick={() => handleCancelOrder(group)} variant="destructive" className="h-8 px-3 text-xs shrink-0 shadow-sm" disabled={submitting}>
            ...
            <Table>
              ...
              <TableBody>
                {displayRows.length === 0 ? (
                  <TableRow><TableCell colSpan={activeTab === "pending" ? 7 : 8} className="text-center py-8 text-gray-500">No rows found.</TableCell></TableRow>
                  ...
                  return (
                    <Fragment key={group.key}>
                      ...
                      >
                        <TableCell colSpan={activeTab === "pending" ? 7 : 8} className="px-4 py-3">
                        ...
                        {activeTab !== "pending" && (
                          <TableCell className="px-4 py-3">
                            <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium ${group.rows.some(r => r.status === 'Order Cancelled') ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}>
                              {group.rows.some(r => r.status === 'Order Cancelled') ? 'Cancelled' : 'Dispatched'}
                            </span>
                          </TableCell>
                        )}
                        <TableCell className="px-4 py-3 text-right">
                          {activeTab === "pending" && (
                            <div className="flex justify-end gap-2" onClick={(e) => e.stopPropagation()}>
                              <Button size="sm" onClick={() => handleCancelOrder(group)} variant="destructive" className="h-8 text-xs shrink-0 shadow-sm" disabled={submitting}>
                                <XCircle className="w-3.5 h-3.5 mr-1.5" />
                                Cancel
                              </Button>
                            </div>
                      ...
                      {isExpanded && (
                        <TableRow>
                          <TableCell colSpan={activeTab === "pending" ? 7 : 9} className="p-0 border-b border-slate-200">
