...
import {
  Loader2, Search, CheckCircle2, AlertCircle, ShieldCheck, ChevronDown, ChevronRight, XCircle, Eye, Download, Building, User
} from "lucide-react"
...
export default function ManagementApprovalPage({ user }) {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      let returnQuery = supabase.from("Material Return").select("*").order("id", { ascending: false })
  ...
  const handleExport = () => {
    const dataToExport = activeTab === "pending" ? entries : historyEntries
    exportToExcel(dataToExport, `ManagementApproval_${activeTab}`)
  }
  ...
  const displayList = useMemo(() => {
    let list = activeTab === "pending" ? entries : historyEntries

    if (filterFirm !== "all") {
      list = list.filter(r => r.firmName === filterFirm)
    }
    if (filterParty !== "all") {
      list = list.filter(r => r["Party Name"] === filterParty)
    }

    if (!searchTerm.trim()) return list
    const t = searchTerm.toLowerCase()
    return list.filter((r) => Object.values(r).some((v) => v?.toString().toLowerCase().includes(t)))
  }, [activeTab, entries, historyEntries, searchTerm, filterFirm, filterParty])

  const firmOptions = useMemo(() => {
    const list = activeTab === "pending" ? entries : historyEntries
    const firms = [...new Set(list.map(r => r.firmName).filter(Boolean))]
    return ["all", ...firms]
  }, [activeTab, entries, historyEntries])

  const partyOptions = useMemo(() => {
    const list = activeTab === "pending" ? entries : historyEntries
    const parties = [...new Set(list.map(r => r["Party Name"]).filter(Boolean))]
    return ["all", ...parties]
  }, [activeTab, entries, historyEntries])
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        ...
        <Button onClick={handleExport} variant="outline" className="flex items-center gap-2 w-fit">
