      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="bg-amber-50 border-amber-100 shadow-sm">
          <CardContent className="p-6 flex justify-between items-center">
            ...
            <div className="h-10 w-10 bg-amber-500 rounded-lg flex items-center justify-center text-white shadow-amber-200 shadow-lg">
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <Card className="w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            ...
            <CardContent className="p-6 space-y-8">
              ...
              <section>
                <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-widest mb-4 pb-2 border-b">Technical Specifications</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  <DetailField label="Alumina %" value={selectedOrder.alumina ? `${selectedOrder.alumina}%` : null} showZero />
              ...
              <section>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  <DetailField label="Type of PI" value={selectedOrder.typeOfPI} />
                  <DetailField label="PI Due Date" value={selectedOrder.piDueDate} />
                  <DetailField label="Payment to Be Taken" value={selectedOrder.paymentToBeTaken} />
                  <DetailField label="Advance %" value={selectedOrder.advance ? `${selectedOrder.advance}%` : null} showZero />
                  <DetailField label="Basic %" value={selectedOrder.basic ? `${selectedOrder.basic}%` : null} showZero />
                  <DetailField label="Retention Payment" value={selectedOrder.retentionPayment} />
                  <DetailField label="Retention %" value={selectedOrder.retentionPercentage ? `${selectedOrder.retentionPercentage}%` : null} showZero />
                  <DetailField label="Lead Time (Retention)" value={selectedOrder.leadTimeRetention ? `${selectedOrder.leadTimeRetention} days` : null} />
                  <DetailField label="Lead Time (Final Payment)" value={selectedOrder.leadTimeFinalPayment ? `${selectedOrder.leadTimeFinalPayment} days` : null} />
              ...
              <section>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  <div className="space-y-1">
                    ...
                    <Badge variant={selectedOrder.status === "New Order" ? "secondary" : "outline"} className="capitalize">
...
const DetailField = ({ label, value, showZero = false }) => {
  const isEmpty = value === null || value === undefined || value === ""
  const isZero = value === 0 || value === "0" || value === "0%"
  
  if (isEmpty || (isZero && !showZero)) return null
  return (
    <div className="space-y-1">
