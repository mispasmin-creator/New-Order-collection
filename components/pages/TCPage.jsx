...
export default function TCPage({ user }) {
  ...
  const fetchData = async () => {
    try {
      ...
      let orQuery = supabase
        .from("ORDER RECEIPT")
        .select('id, "DO-Delivery Order No.", "Rate Of Material", "TC Required", "Firm Name"')
      ...
      orderReceiptData?.forEach(row => {
        ...
        if (row.id) {
          tcRequiredMap.set(row.id, row["TC Required"])
          firmMap.set(row.id, row["Firm Name"])
        }
      })
  ...
  const displayOrders = useMemo(() => {
    let source = activeTab === "pending" ? orders : historyOrders
    
    if (filterFirm !== "all") {
      source = source.filter(order => order.firmName === filterFirm)
    }
    if (filterParty !== "all") {
  ...
  const firmOptions = useMemo(() => {
    ...
    const firms = [...new Set(source.map(order => order.firmName).filter(Boolean))]
    return ["all", ...firms]
  }, [activeTab, orders, historyOrders])

  const partyOptions = useMemo(() => {
    const source = activeTab === "pending" ? orders : historyOrders
    const parties = [...new Set(source.map(order => order.partyName).filter(Boolean))]
    return ["all", ...parties]
  }, [activeTab, orders, historyOrders])
  ...
  const handleExport = () => {
    const dataToExport = activeTab === "pending" ? orders : historyOrders
    exportToExcel(dataToExport, `TC_${activeTab}`)
  }

  const handleOpenModal = (order) => {
    setSelectedGroup({ invoiceNo: order.billNumber, partyName: order.partyName, rows: [order] })
    setFormData({ tcFile: null, tcRequired: order.tcRequired || "No" })
  }
  ...
  const handleSubmit = async () => {
    ...
    if (formData.tcRequired === "Yes" && !formData.tcFile) {
      toast({ variant: "destructive", title: "Upload required", description: "Please upload a Test Certificate file before submitting when TC is required" })
      return
    }
    ...
    try {
      ...
      await Promise.all(
        selectedGroup.rows.map(row =>
          supabase.from("DISPATCH").update({ 
            "Trust Certificate Made": tcFileUrl,
            "TC Required": formData.tcRequired
          }).eq("id", row.id)
        )
      )

      // Update ORDER RECEIPT as well
      await Promise.all(
        selectedGroup.rows.map(row => 
          supabase.from("ORDER RECEIPT").update({ "TC Required": formData.tcRequired }).eq("id", row.po_id)
        )
      )
            ...
            <CardContent className="p-4">
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <Label className="text-sm">TC Required</Label>
                  <Select
                    value={formData.tcRequired}
                    onValueChange={(value) => setFormData(prev => ({ ...prev, tcRequired: value }))}
                  >
                ...
                <div className="border-t pt-4 flex flex-col gap-3">
                  ...
                  <Button
                    ...
                    disabled={submitting || (formData.tcRequired === "Yes" && !formData.tcFile)}
                  >
